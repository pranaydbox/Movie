...
window.onload = async function () {
    ...
    $.post(host+"/bookings/getseatstatus", { pattern: resstr }, (xhr, status, responseText) => {
    ...
    moviedata=await $.get(host+"/movies/getmovie/" + arr[0]);
    theatredata=await $.get(host+"/theatres/getsingletheatre/" + arr[1]);
...
function confirmbooking() {
    ...
    var arr = window.location.hash.substring(1).split(":");
    $.post(host+"/bookings/booknow", {price:cost,movieId: arr[0], theatreId: arr[1], bookings: count, curremail: localStorage.getItem("currentLoginUser"), changedseats: ar, pattern: window.location.hash.substring(1) }, (xhr, status, responseText) => {
